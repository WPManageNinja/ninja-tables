    ...
    <el-container class="ninja-add-table">
        <el-aside v-if="!table.ID" class="ninja-tables-aside">
            ...
            >
                ...
                <el-menu-item v-if="has_woo" @click="activeTabName = 'woo_reviews'" index='woo_reviews'>
                    <div class="tab-label">
                        <div class="active-bar"></div>
                        <div class="tab-title">
                            <span>{{$t('WooCommerce Reviews')}}</span>
                            <img :src="assetUrl('icons/active-arrow.svg')"/>
                        </div>
                    </div>
                </el-menu-item>

                <el-menu-item v-if="has_fluent_cart" @click="activeTabName = 'fluent_cart'" index='fluent_cart'>
                    <div class="tab-label">
                        <div class="active-bar"></div>
                        <div class="tab-title">
                            <span>{{$t('FluentCart Table')}}</span>
                            <img :src="assetUrl('icons/active-arrow.svg')"/>
                        </div>
                    </div>
                </el-menu-item>
        ...
        <el-main class="ninja-tables-main">
            ...
            <template v-else-if="activeTabName === 'fluent_cart'">
                <FcProducts
                    :tableCreated="fireTableCreated"
                    @modalClose="closeModal"
                />
...
<!--                    <h3>Upgrade to pro for using WooCommerce Integration</h3>-->
            ...
            <template v-else-if="activeTabName === 'woo_reviews'">
                <WooReviews
                    v-if="activated_features.woocommerce_table"
                    :tableCreated="fireTableCreated"
                    @modalClose="closeModal"
                ...
                <div v-else-if="has_woo && !hasPro" class="ninja_no_woo">
                    <PremiumNotice title="Construct Table from Your WooCommerce Product Reviews">
                        <template #default>
                            <p class="text-[14px] font-[400] text-[#525866]">
                                {{ $t('This is a Premium feature. Displays product Reviews in a searchable, sortable with Ninja Tables.') }}
<!--                                <a class="nt-link" target="_blank" href="https://ninjatables.com/docs/woocommerce-integration/">-->
<!--                                    {{ $t('View Documentation') }}-->
<!--                                </a>-->
<!--                                {{$t('or')}}-->
...
<script type="text/babel">
    ...
    import WooReviews from "./DataProviders/WooReviews.vue";
    import FcProducts from './DataProviders/FcProducts.vue';
    ...
    export default {
        ...
        data() {
            return {
                ...
                has_fluent_cart: !!window.ninja_table_admin.has_fluentCart,
                initialData : {},
            }
        },
        methods: {
