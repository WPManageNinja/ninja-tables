<template>
    <div v-loading="loading" class="wp_posts_dynamic_field">
        ...
        <div class="flex flex-col mb-2">
            ...
            <div v-else>
                <el-input
                    v-if="selectedField.key === 'shortcode'"
        ...
        <div v-if="column.wp_post_custom_data_key === 'product_average_rating'">
            <div class="flex flex-row mt-4 gap-4">
                <div class="flex flex-col flex-1">
                    ...
                    <div class="border-solid border border-[lightgray] px-[5] py-1 w-full rounded-lg">
                        <ColorPicker v-model="filledStarColor" />
                    </div>
                </div>
                <div class="flex flex-col flex-1">
                    <label class="nt-form-label">
                    ...
                    <div class="border-solid border border-[lightgray] px-[5] py-1 w-full rounded-lg">
                        <ColorPicker v-model="emptyStarColor" />
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-row mt-4 gap-4">
                <div class="flex flex-col flex-1">
                    <label class="nt-form-label">
                        ...
                        <el-tooltip class="item" placement="bottom-start" effect="light">
                            <template #content>
                                <h3>{{$t('No Review Text')}}</h3>
                                <p>{{$t('Text to display when there are no reviews')}}</p>
                            </template>
                            <el-icon class="tooltip-icon-color"><InfoFilled /></el-icon>
                        </el-tooltip>
                    </label>
                    <NinjaInput
                        :placeholder="$t('No reviews yet')"
                        v-model="emptyReviewText"
                    />
                </div>
                <div class="flex flex-col flex-1">
                    <label class="nt-form-label">
                        ...
                        <el-tooltip class="item" placement="bottom-start" effect="light">
                            <template #content>
                                <h3>{{$t('Show Rating')}}</h3>
                                <p>{{$t('Enable this to show rating value alongside stars')}}</p>
                            </template>
                            <el-icon class="tooltip-icon-color"><InfoFilled /></el-icon>
                        </el-tooltip>
                    </label>
                    <el-checkbox v-model="showRatingText" :true-value="'yes'" :false-value="'no'" :label="$t('Show Rating Value')"></el-checkbox>
                </div>
            </div>
        </div>
        ...
        <template v-if="column.wp_post_custom_data_key === 'post_title' || (column.source_type === 'custom' && column.wp_post_custom_data_type === 'featured_image')">
            <el-form-item class="flex flex-col">
                ...
                <el-checkbox v-if="column.wp_post_custom_data_key === 'post_author'" v-model="column.permalinked" :true-value="'yes'" :false-value="'no'" value="yes" :label="$t('Link to Author Permalink')"></el-checkbox>
                <el-checkbox v-else v-model="column.permalinked" :true-value="'yes'" :false-value="'no'" value="yes" :label="$t('Link to post permalink')"></el-checkbox>
                <el-checkbox v-model="column.permalink_target" :true-value="'_blank'" :false-value="''" value="_blank" :label="$t('Open link to new tab')"></el-checkbox>
            </el-form-item>
        </template>
        ...
        <template v-else-if="column.source_type === 'tax_data'">
            <div class="flex flex-col mt-4">
                ...
                <el-checkbox v-model="column.permalinked" :true-value="'yes'" :false-value="'no'" value="yes" :label="$t('Link to Taxonomy')"></el-checkbox>
            </div>
                    ...
                    <label class="nt-form-label mb-2">
                        ...
                        <el-tooltip class="item" placement="bottom-start" effect="light">
                            <template #content>
                                ...
                                <p>{{$t('Enable this if you want to make the taxonomies as table filter action. So when user click on those filters then they will see only those type of posts.')}}</p>
                            </template>
                            <el-icon class="tooltip-icon-color"><InfoFilled /></el-icon>
                        </el-tooltip>
                    </label>
                    <el-checkbox v-model="column.filter_permalinked" :true-value="'yes'" false-label="" value="yes" :label="$t('Make Taxonomies as Table Filter')"></el-checkbox>
                </div>

                <div v-if="column.filter_permalinked !== 'yes'" class="flex flex-col mt-4">
                    <label class="nt-form-label mb-2">
...
<script type="text/babel">
    ...
    export default {
        ...
        computed: {
            ...
            filledStarColor: {
                get() {
                    return this.column.filled_star_color || '#ffd700';
                },
                set(value) {
                    this.column.filled_star_color = value;
                }
            },
            emptyStarColor: {
                get() {
                    return this.column.empty_star_color || '#e0e0e0';
                },
                set(value) {
                    this.column.empty_star_color = value;
                }
            },
            showRatingText: {
                get() {
                    return this.column.show_rating_text || 'yes';
                },
                set(value) {
                    this.column.show_rating_text = value;
                }
            },
            emptyReviewText: {
                get() {
                    return this.column.empty_review_text || this.$t('No rating');
                },
                set(value) {
                    this.column.empty_review_text = value;
                }
            }
        },
        components: {
