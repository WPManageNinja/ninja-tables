    ...
    <div style="margin: 1px;" class="ntb-datas-wrapper">
        ...
        <span v-else-if="item.data.type === 'button'" class="hover-item"
            ...
            <a @click.prevent :href="`${item.data.style.url}`"
                ...
                <el-button class="ntb-el-button" type="primary"
                           ...
                           :data-button-data="JSON.stringify(item.data)"
        ...
        <span v-else-if="item.data.type === 'image'">
           <a class="hover-item"
              ...
              :style="[displayBlock]"
              :rel="linkAttributes"
              v-if="JSON.parse(JSON.stringify(item.data.style.link))"
           >
                 <img
                     :src="item.data.value"
    ...
    methods: {
        ...
        mouseOver(event) {
            if (this.item.data.style.isHover === true) {
                ...
                event.currentTarget.style.background = this.item.data.style.hoverBackgroundColor;
                event.currentTarget.style.color = hover;
                event.currentTarget.style.border = this.item.data.style.hoverBorderSize + 'px solid' + this.item.data.style.hoverBorderColor;
                event.currentTarget.style.transform = 'scale(' + this.item.data.style.transition + ')';
            }
        },
        mouseOut(event) {
            if (this.item.data.style.isHover === true) {
                let textColor = this.item.data.style.color;
                let unHoverText = (textColor === '') ? this.setting.global_styling.options.color.value : textColor;
                let iconColor = this.item.data.style.iconColor;
                let unHoverIcon = (iconColor === '') ? this.setting.global_styling.options.color.value : iconColor;

                event.currentTarget.style.background = this.item.data.style.backgroundColor;
                event.currentTarget.style.color = unHoverText;
                event.currentTarget.style.border = this.item.data.style.borderSize + 'px solid' + this.item.data.style.borderColor;
                event.currentTarget.style.transform = 'scale(1)';

                // Reset icon color
                const iconElement = event.currentTarget.querySelector('.svgIcon');
                if (iconElement) {
                    iconElement.style.background = unHoverIcon;
                }
            }
        }
    },
