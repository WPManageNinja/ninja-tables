...
export default function BlockPreview({
    ...
    const getTableClasses = () => {
        ...
        if (isTruthy(settings.hide_header_row)) classes.push('ninjatable_hide_header_row');
        if (isTruthy(settings.hide_all_borders)) classes.push('hide_all_borders');
        if (hasPro) classes.push('ninja_table_pro');
        if (settings.search_position) classes.push(`ninja_search_${settings.search_position}`);
        if (isTruthy(settings.hide_responsive_labels)) classes.push('nt_hide_breakpoint_labels');
        if (isTruthy(settings.nt_search_full_width)) classes.push('nt_search_full_width');
        ...
        return (
            <div className="ninja_design_wrapper">
                ...
                <div className="design_preview" style={{background: 'white', padding: '10px 20px'}}>
                    {isTruthy(tableConfig?.settings?.show_title) && tableConfig?.table?.post_title && (
                        <h3 className="table_title footable_title">{tableConfig.table.post_title}</h3>
                    )}

                    {isTruthy(tableConfig?.settings?.show_description) && tableConfig?.table?.post_content && (
                        <div
