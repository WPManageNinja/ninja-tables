(function ($) {
    ...
    */
    ...
    // Add button hover functionality for frontend
    function initButtonHoverEffects() {
        $(document).on('mouseover', '.ntb-el-button, .ntb-el-button *', function(e) {
            const button = $(this).closest('.ntb-el-button');
            if (button.length && button.data('hover-enabled') === true) {
                const hoverData = button.data('hover-styles');
                if (hoverData) {
                    button.css({
                        'background': hoverData.hoverBackgroundColor,
                        'color': hoverData.hoverColor,
                        'border': hoverData.hoverBorderSize + 'px solid ' + hoverData.hoverBorderColor,
                        'transform': 'scale(' + hoverData.transition + ')'
                    });
                }
            }
        });

        $(document).on('mouseout', '.ntb-el-button, .ntb-el-button *', function(e) {
            const button = $(this).closest('.ntb-el-button');
            if (button.length && button.data('hover-enabled') === true) {
                const defaultData = button.data('default-styles');
                if (defaultData) {
                    button.css({
                        'background': defaultData.backgroundColor,
                        'color': defaultData.color,
                        'border': defaultData.borderSize + 'px solid ' + defaultData.borderColor,
                        'transform': 'scale(1)'
                    });
                    
                    // Reset icon color
                    const iconElement = button.closest('.ntb-datas-wrapper').find('.svgIcon');
                    if (iconElement.length && defaultData.iconColor) {
                        iconElement.css('background', defaultData.iconColor);
                    }
                }
            }
        });
    ...
    function setupButtonHoverData() {
        $('.ntb-el-button').each(function() {
            const button = $(this);
            const buttonDataAttr = button.attr('data-button-data');
            
            if (buttonDataAttr) {
                try {
                    const buttonData = JSON.parse(buttonDataAttr);
                    
                    if (buttonData && buttonData.style && buttonData.style.isHover === true) {
                        button.data('hover-enabled', true);
                        button.data('hover-styles', {
                            hoverBackgroundColor: buttonData.style.hoverBackgroundColor,
                            hoverColor: buttonData.style.hoverColor || buttonData.style.color,
                            hoverBorderColor: buttonData.style.hoverBorderColor,
                            hoverBorderSize: buttonData.style.hoverBorderSize,
                            transition: buttonData.style.transition
                        });

                        button.data('default-styles', {
                            backgroundColor: buttonData.style.backgroundColor,
                            color: buttonData.style.color,
                            borderColor: buttonData.style.borderColor,
                            borderSize: buttonData.style.borderSize,
                            iconColor: buttonData.style.iconColor
                        });
                    }
                } catch (e) {
    ...
    $(document).ready(function ($) {
        ...
        $('.ntb_table_wrapper').each(function () {
            ...
            if (isResponsive === true || isResponsive === 'true') {
                ...
                // Trigger table loaded event after responsive setup
                // setTimeout(function() {
                //     $(document).trigger('ninja_table_loaded');
                // }, 500);
