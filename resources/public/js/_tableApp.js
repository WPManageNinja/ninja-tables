...
export default {
    ...
    // It used to woocomece and fluent table bulk comparison checkboxes
    addClasses(tableConfig, fooTable) {
        if (['wp_fct', 'wp_woo'].includes(tableConfig.provider) && tableConfig.render_type === 'ajax_table') {
            fooTable.rows.all.forEach(function (row, index) {
                $(row.$el).attr('data-row_id', index);
                $(row.$el).addClass('nt_row_id_'+index);
            });
        }
    },
    initTable($table, tableConfig) {
        ...
        $table.on('ready.ft.table', (e, fooTable) => {
            ...
            .on('postdraw.ft.table', (e, fooTable) => {
                ...
                try {
                    this.addClasses(tableConfig, fooTable);
                    Event.trigger(
    ...
    getNinjaTableConfig(tableConfig) {
        ...
        jQuery.each(tableConfig.columns, (columnIndex, column) => {
            ...
            column.formatter = function (value, options, rowData) {
                ...
                if (column.key === 'nt_product_compare') {
                    value = '<input type="checkbox" class="ninja-compare-checkbox"/>';
                }
